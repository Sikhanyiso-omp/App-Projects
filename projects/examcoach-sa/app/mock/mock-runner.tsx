...
export default function MockRunner({ questions }: { questions: MockQuestion[] }) {
  ...
  const finish = useCallback(async () => {
    ...
    const finalScore = questions.reduce((sum, item, index) => sum + (finalAnswers[index] === item.correctIndex ? item.marks : 0), 0);
    const finalReadiness = totalMarks ? Math.round(finalScore / totalMarks * 100) : 0;
    setFinished(true);
    const response = await fetch("/api/attempts", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ assessmentType: "mock", score: finalScore, total: totalMarks, readiness: finalReadiness, answers: finalAnswers }) });
    if (response.ok) setSaved(true);
  }, [questions, totalMarks]);
  useEffect(() => { if (!started || finished) return; const timer = window.setInterval(() => setSeconds((value) => { if (value <= 1) { window.clearInterval(timer); void finish(); return 0; } return value - 1; }), 1000); return () => window.clearInterval(timer); }, [finish, started, finished]);
  ...
  if (finished) return <section className="mock-results"><div><p className="eyebrow">Mock complete · {saved ? "result saved" : "saving result"}</p><h2>Your current readiness is <em>{readiness}%</em>.</h2><p>You scored {score} out of {totalMarks} marks. Review every explanation below, then repeat your weakest lesson before trying again.</p><div className="result-actions"><Link className="button button-primary" href="/learn">Review lessons →</Link><button className="button button-ghost" onClick={() => { finishingRef.current = false; setSaved(false); setAnswers(questions.map(() => -1)); setCurrent(0); setSeconds(1800); setFinished(false); }}>Try again</button></div></div><div className="review-list">{questions.map((item, index) => <article className={answers[index] === item.correctIndex ? "review-correct" : "review-wrong"} key={item.id}><span>{answers[index] === item.correctIndex ? "✓" : "×"}</span><div><b>{item.topic}</b><p>{item.prompt}</p><small>{item.explanation}</small></div></article>)}</div></section>;
